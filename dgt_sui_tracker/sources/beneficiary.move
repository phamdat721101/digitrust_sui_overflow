...
module swap::dgt_treasury {
    ...
    /// Transfers withdrew fee coins to the dgt_treasury.
    public entry fun withdraw<X, Y>(
        global: &mut Global,
        ctx: &mut TxContext
    ) {
        assert!(!implements::is_emergency(global), ERR_EMERGENCY);
        assert!(implements::dgt_treasury(global) == tx_context::sender(ctx), ERR_NO_PERMISSIONS);
